...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val userViewModel : UserViewModel by viewModels {
            ...
            UserViewModel.UserViewModelFactory(userRepository = UserRepositoryImpl(database.userDao(),database.addressDao()))
        }

        val addressViewModel : AddressViewModel by viewModels {
            val database = AppDatabase.getDatabase(applicationContext)
            AddressViewModel.AddressViewModelFactory(addressRepository = AddressRepositoryImpl(database.addressDao()))
        }

        setContent {
            MyGoalsAppTheme {
                OutsideNavigation(MaterialTheme.colorScheme, MaterialTheme.typography, userViewModel, addressViewModel)
            }
        }
    }
}
